...
class OwnerAuthorizationSeeder extends Seeder
{
    /**
     * Assign permissions, menus, and menu positions for Owner role.
     * Owner has read access to all features with limited write capabilities.
     */
    public function run(): void
    {
        $this->command->info('🔧 Setting up Owner Authorization...');
        ...
        $this->command->info("✅ Assigned {$role->permissions->count()} permissions to Owner");

        // ===== STEP 3: ASSIGN MENUS =====
        // Owner sees all menus except Konfigurasi and its children
        ...
        $this->command->info("✅ Assigned {$menus->count()} menus to Owner");

        // ===== STEP 4: ASSIGN MENU POSITIONS =====
        $excludeNames = $excludeIds->isNotEmpty()
            ? Menu::whereIn('id', $excludeIds)->pluck('name')->toArray()
            : [];
        ...
        $this->command->info('✅ Owner Authorization setup completed!');
        $this->command->info('');
    }
}
